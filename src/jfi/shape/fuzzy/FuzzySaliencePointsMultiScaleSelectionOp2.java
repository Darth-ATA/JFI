 ...
 */
public class FuzzySaliencePointsMultiScaleSelectionOp2 extends FuzzySaliencePointsSelectionOp {
    ...
    public FuzzySaliencePointsMultiScaleSelectionOp2(int window_size_curvacity, double alpha_curvacity, int window_size_maxima,
    ...
    public FuzzySaliencePointsMultiScaleSelectionOp2(){
        super();
    }
     ...
     * @return the error between the contour segment and the line defined by points.
     */
    private static double errorSegments2(FuzzyContour contour, List<Iterable.FuzzyItem> points) {
        double maxDistance = -Double.MAX_VALUE;
        double iDistance;
        double accumulatedError = 0;
        Iterable.FuzzyItem startPoint;
        Iterable.FuzzyItem endPoint;

        for (int i=0; i < points.size() - 1; i++) {
            startPoint = points.get(i);
            endPoint = points.get(i + 1);
            for (Iterable.FuzzyItem point2D : contour) {
                iDistance = absDistanceBetweenLineAndPoint((Point2D) startPoint.getElement(), (Point2D) endPoint.getElement(),
                        (Point2D) point2D.getElement());
                if (iDistance > maxDistance) {
                    maxDistance = iDistance;
                }
            }
     ...
     * @return the nearest point.
     */
    private static Iterable.FuzzyItem nearestPoint(FuzzyContour contour, Iterable.FuzzyItem point){
        double pointMinDistance = Double.MAX_VALUE;
        double iPointDistance;
        Iterable.FuzzyItem nearestPoint = point;
        for (Iterable.FuzzyItem iPoint : contour) {
            iPointDistance = euclideanDistance2D((Point2D) point.getElement(), (Point2D) iPoint.getElement());
            if (iPointDistance < pointMinDistance) {
                nearestPoint = iPoint;
                pointMinDistance = iPointDistance;
            }
    ...
    /**
     * It finds the nearest point in a set of points.
     * @param fuzzyItems the list whose find the nearest point.
     * @param point the point for find the nearest point.
     * @return the nearest point.
     */
    private static Iterable.FuzzyItem nearestPoint(List<Iterable.FuzzyItem> fuzzyItems, Iterable.FuzzyItem point){
        double pointMinDistance = Double.MAX_VALUE;
        double iPointDistance;
        Iterable.FuzzyItem nearestPoint = point;
        for (Iterable.FuzzyItem iPoint : fuzzyItems) {
            iPointDistance = euclideanDistance2D((Point2D) point.getElement(), (Point2D) iPoint.getElement());
            if (iPointDistance < pointMinDistance) {
                nearestPoint = iPoint;
                pointMinDistance = iPointDistance;
     ...
     * @return each scale with their points trajectory.
     */
    private Map<Double, Map<Iterable.FuzzyItem, List<Iterable.FuzzyItem>>> pointsTrajectory2(Map<Double, FuzzyContour> scalesSaliencePoints){
        Map<Double, Map<Iterable.FuzzyItem, List<Iterable.FuzzyItem>>> pointsTrajectory = new ConcurrentHashMap<>();
        List<Double> sigmaList = new ArrayList<>(scalesSaliencePoints.keySet());
        Collections.sort(sigmaList);
        ...
        for (int i = 0; i < sigmaList.size() - 1; i++){
            double currentSigma = sigmaList.get(i);
            double nextSigma = sigmaList.get(i+1);
            Map<Iterable.FuzzyItem, List<Iterable.FuzzyItem>> pointsCorrespondence = new ConcurrentHashMap<>();
            FuzzyContour currentPoints = scalesSaliencePoints.get(currentSigma);
            FuzzyContour nextPoints = scalesSaliencePoints.get(nextSigma);
            currentPoints.forEach(fuzzyPoint -> {
                Iterable.FuzzyItem nearestPoint = nearestPoint(nextPoints, fuzzyPoint);
                if (pointsCorrespondence.containsKey(nearestPoint)){
                    List<Iterable.FuzzyItem> nearestCorrespondence = pointsCorrespondence.get(nearestPoint);
                    nearestCorrespondence.add(fuzzyPoint);
                    pointsCorrespondence.replace(nearestPoint, nearestCorrespondence);
                }
                else {
                    List<Iterable.FuzzyItem> nearestCorrespondence = new ArrayList<>();
                    nearestCorrespondence.add(fuzzyPoint);
                    pointsCorrespondence.put(nearestPoint, nearestCorrespondence);
                }
            });
     ...
     * @return the point correspondence in the next scale.
     */
    private Iterable.FuzzyItem pointScaleCorrespondence2(Map<Double, Map<Iterable.FuzzyItem, List<Iterable.FuzzyItem>>> pointsTrajectoryTree,
                                                         Double sigma, Iterable.FuzzyItem point){
        if (pointsTrajectoryTree.containsKey(sigma)){
            Map<Iterable.FuzzyItem, List<Iterable.FuzzyItem>> correspondencePoints = pointsTrajectoryTree.get(sigma);
            return nearestPoint(correspondencePoints.get(point), point);
        }
        else {
            return point;
     ...
     * @return the point correspondence in the original contour.
     */
    private List<Iterable.FuzzyItem> pointOriginalScaleTrajectory2(Map<Double, Map<Iterable.FuzzyItem, List<Iterable.FuzzyItem>>> pointsTrajectoryTree, Double sigma, Iterable.FuzzyItem point){
        List<Iterable.FuzzyItem> originalScaleCorrespondence = new ArrayList<>();
        List<Double> sigmaList = new ArrayList<>(pointsTrajectoryTree.keySet());
        Collections.sort(sigmaList);
        originalScaleCorrespondence.add(point);
        for (int i = sigmaList.indexOf(sigma); i >= 0; i--) {
            double iSigma = sigmaList.get(i);
            Map<Iterable.FuzzyItem, List<Iterable.FuzzyItem>> correspondencePoints = pointsTrajectoryTree.get(iSigma);
            List<Iterable.FuzzyItem> listOfPoints = new ArrayList<>(correspondencePoints.keySet());
            List<Iterable.FuzzyItem> newOriginalScaleCorrespondence = new ArrayList<>();
            for (Iterable.FuzzyItem pointCorrespondence : originalScaleCorrespondence) {
                newOriginalScaleCorrespondence.addAll(correspondencePoints.get(nearestPoint(listOfPoints, pointCorrespondence)));
            }
            originalScaleCorrespondence = newOriginalScaleCorrespondence;
        }
    ...
    private List<Iterable.FuzzyItem> orderPointsLikeContour2(FuzzyContour contour, List<Iterable.FuzzyItem> points) {
        List<Iterable.FuzzyItem> orderedPointsOriginal = new ArrayList<>();
        contour.forEach(point -> {
            if (points.contains(point))
                orderedPointsOriginal.add(point);
        });
    ...
    private static List<Iterable.FuzzyItem> getFuzzySegment(FuzzyContour fuzzyContour, Iterable.FuzzyItem start, Iterable.FuzzyItem end) {
        Iterable.FuzzyItem nearStart = nearestPoint(fuzzyContour, start);
        Iterable.FuzzyItem nearEnd = nearestPoint(fuzzyContour, end);

        Contour contour = fuzzyContour.getContourReferenceSet();
        Point2D startPoint = (Point2D) nearStart.getElement();
        Point2D endPoint = (Point2D) nearEnd.getElement();
        int startIndex = contour.indexOf(startPoint);
        int endIndex = contour.indexOf(endPoint);

        boolean isClockwise = startIndex <= endIndex;
        List<Point2D> segment = contour.getSegment(startIndex, endIndex, isClockwise);

        List<Iterable.FuzzyItem> fuzzySegment = new ArrayList<>();
        for (Iterable.FuzzyItem<Point2D> pointFuzzyItem : fuzzyContour) {
            if (segment.contains(pointFuzzyItem.getElement())) {
                fuzzySegment.add(pointFuzzyItem);
            }
        }
        return fuzzySegment;
    }
     ...
     * @return the points that are saliency points after the multi scale method.
     */
    private List<Iterable.FuzzyItem> getSalienceMultiScale2(Iterable.FuzzyItem start, Iterable.FuzzyItem end,
                                                Map<Double, FuzzyContour> sigmaScalesSaliencePoints, double threshold,
                                                Map<Double, Map<Iterable.FuzzyItem, List<Iterable.FuzzyItem>>> pointsTrajectoryTree){
        if (sigmaScalesSaliencePoints.size() > 0){
            Set<Iterable.FuzzyItem> saliencePointsSet = new HashSet<>();

            Double maxSigma = sigmaScalesSaliencePoints.keySet().stream().max(Double::compare).get();
            FuzzyContour salienceFuzzyContour = sigmaScalesSaliencePoints.get(maxSigma);
            List<Iterable.FuzzyItem> segmentSaliencePoints;
            if (start == null || end == null){
                segmentSaliencePoints = new ArrayList<>();
                salienceFuzzyContour.forEach(segmentSaliencePoints::add);
            }
            else {
                segmentSaliencePoints = getFuzzySegment(salienceFuzzyContour, start, end);
            }

            if (segmentSaliencePoints.size() > 1) {
                for (int i = 0; i < segmentSaliencePoints.size(); i++) {
                    Iterable.FuzzyItem startPoint = segmentSaliencePoints.get(i % segmentSaliencePoints.size());
                    Iterable.FuzzyItem endPoint = segmentSaliencePoints.get((i + 1) % segmentSaliencePoints.size());

                    List<Iterable.FuzzyItem> startPointsOriginal = pointOriginalScaleTrajectory2(pointsTrajectoryTree, maxSigma, startPoint);
                    startPointsOriginal = orderPointsLikeContour2(salienceFuzzyContour, startPointsOriginal);
                    List<Iterable.FuzzyItem> endPointsOriginal = pointOriginalScaleTrajectory2(pointsTrajectoryTree, maxSigma, endPoint);
                    endPointsOriginal = orderPointsLikeContour2(salienceFuzzyContour, endPointsOriginal);

                    List<Iterable.FuzzyItem> pointsOriginal = new ArrayList<>(startPointsOriginal);
                    pointsOriginal.addAll(endPointsOriginal);

                    Map<Double, FuzzyContour> iSigmaScales = new ConcurrentHashMap<>(sigmaScalesSaliencePoints);

                    double segmentsError = errorSegments2(salienceFuzzyContour, pointsOriginal);
                    saliencePointsSet.addAll(startPointsOriginal);
                    if (segmentsError > threshold) {
                        iSigmaScales.remove(maxSigma);
                        startPoint = pointScaleCorrespondence2(pointsTrajectoryTree, maxSigma, startPoint);
                        endPoint = pointScaleCorrespondence2(pointsTrajectoryTree, maxSigma, endPoint);
                        saliencePointsSet.addAll(getSalienceMultiScale2(startPoint, endPoint,
                                iSigmaScales, threshold, pointsTrajectoryTree));
                    }
                    saliencePointsSet.addAll(endPointsOriginal);
                }
                ...
                Map<Double, FuzzyContour> iSigmaScales = new ConcurrentHashMap<>(sigmaScalesSaliencePoints);
                iSigmaScales.remove(maxSigma);
                saliencePointsSet.addAll(getSalienceMultiScale2(start, end,
                        iSigmaScales, threshold, pointsTrajectoryTree));
                Iterable.FuzzyItem endPoint = segmentSaliencePoints.get(0);
                saliencePointsSet.addAll(pointOriginalScaleTrajectory2(pointsTrajectoryTree, maxSigma, endPoint));
            }
            else if (sigmaScalesSaliencePoints.size() > 1){
                Map<Double, FuzzyContour> iSigmaScales = new ConcurrentHashMap<>(sigmaScalesSaliencePoints);
                iSigmaScales.remove(maxSigma);
    ...
    /**
     * Calculate the min salience error point.
     * @param fuzzyContour whose calculate error
     * @param salienceMultiScale the list of salience points selected.
     * @return the point and his associate error.
     */
    private static Pair<Iterable.FuzzyItem, Double> minSalienceError(FuzzyContour fuzzyContour, List<Iterable.FuzzyItem> salienceMultiScale){
        Pair<Iterable.FuzzyItem, Double> minError = new Pair<>(salienceMultiScale.get(0), Double.MAX_VALUE);
        double iError;
        List<Point2D> originalSegment;
        Point2D iStartPoint;
        Iterable.FuzzyItem iCurrentPoint;
        Point2D iEndPoint;
        for(int i = 0; i < salienceMultiScale.size(); i++){
            iStartPoint = (Point2D) salienceMultiScale.get((i-1) < 0 ? salienceMultiScale.size() - 1 : (i-1)).getElement();
            iCurrentPoint = salienceMultiScale.get(i);
            iEndPoint = (Point2D) salienceMultiScale.get((i+2) % salienceMultiScale.size());
            originalSegment = fuzzyContour.getContourReferenceSet().getSegment(iStartPoint, iEndPoint);
            iError = errorSegments(originalSegment, iStartPoint, iEndPoint);
            if (iError < minError.getRight()){
                minError = new Pair<>(iCurrentPoint, iError);
            }
     ...
     * @return
     */
    private List<Iterable.FuzzyItem> minSalienceMultiScale2(FuzzyContour fuzzyContour, List<Iterable.FuzzyItem> salienceMultiScale, double threshold){
        List<Iterable.FuzzyItem> minSalienceMultiScale = new ArrayList<>(salienceMultiScale);
        Pair<Iterable.FuzzyItem, Double> iMinError = minSalienceError(fuzzyContour, minSalienceMultiScale);
        Iterable.FuzzyItem removablePoint;
        while(iMinError.getRight() <= threshold && minSalienceMultiScale.size() > 2){
            removablePoint = iMinError.getLeft();
            minSalienceMultiScale.remove(removablePoint);
            iMinError = minSalienceError(fuzzyContour, minSalienceMultiScale);
        }
        /*
        It establish a min distance between the points and remove the relevant points that do not have this minimum distance between them
        For the moment I have not encounter eny need for use it.
     ...
     * @return
     */
    public FuzzyContour apply2(Contour contour, double sigma, double threshold, double alphacut) {
    ...
    public FuzzyContour apply(Contour contour, double sigma, double threshold, double alphacut) {
        ...
        FuzzyContour firstFuzzyContour = new FuzzyContour();
        for (int i = 0; i < sigmaScales.size(); i++){
            double iSigma = sigmaScales.get(i);
            Contour iContour = contour.gaussianFiltering(iSigma);
            FuzzyContour iSaliencePoints = super.apply(iContour, alphacut);
            if (i == 0)
                firstFuzzyContour = iSaliencePoints;
            scalesFuzzyContours.put(iSigma, iSaliencePoints);
        }

        // Calculates the points trajectories in each scale.
        Map<Double, Map<Iterable.FuzzyItem, List<Iterable.FuzzyItem>>> pointsTrajectoryTree = pointsTrajectory2(scalesFuzzyContours);
        List<Iterable.FuzzyItem> minSalience = getSalienceMultiScale2(null, null,
                scalesFuzzyContours, threshold, pointsTrajectoryTree);
        //minSalience = minSalienceMultiScale2(firstFuzzyContour, minSalience, threshold);

        System.out.println("Los puntos multiescala son: " + minSalience.toString());
        FuzzyContour saliencePoints = new FuzzyContour("Contour.Salience Point Multi Scale");
        for (Iterable.FuzzyItem item : minSalience) {
            saliencePoints.add((Point2D) item.getElement(), item.getDegree());
        }
        return saliencePoints;
    }
